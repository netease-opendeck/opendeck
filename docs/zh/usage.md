...
| `OPENCLAW_SKILL_EXECUTION_PATH` | 否 | 执行记录文件相对路径（默认：`memory/skill-execution.jsonl`） |
| `PORT` | 否 | 后端端口（默认：3000） |
...
- **没有任务或文件** — 执行数据来自 `memory/skill-execution.jsonl`，需确保 OpenClaw（及若使用 skill-tracker）会写入该路径。
