...
| `PORT` | 否 | 后端端口（默认：19520） |

在运行 `./scripts/install.sh -y` 前，将 `backend/.env.example` 复制为 `backend/.env` 并设置 `OPENCLAW_ROOT`。

## 界面概览

- **技能** — 列出 `OPENCLAW_ROOT/OPENCLAW_SKILLS_PATH` 下的技能，点击可查看 SKILL.md 与元数据。
- **文件管理** — 聚合技能执行记录中的文件，以及 `OPENCLAW_ROOT/workspace/memory/reflection` 下的 Markdown 文件；可按目录浏览、搜索、按类型筛选、预览内容。
...
- `GET /api/files`、`GET /api/files/tree`、`GET /api/files/stats`、`GET /api/files/content` — 文件相关（其中 `files/tree/stats` 会返回 `warnings`，用于提示 reflection 扫描中的部分失败）  
...
- **安装后服务未就绪** — 使用 `pm2 logs deck-backend` 查看日志，并检查 `backend/.env` 配置。
