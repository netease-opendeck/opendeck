...
```json
{
  ...
  "messages": [
    {
      "role": "user|assistant",
      "content": "string (the ORIGINAL message content — preserve the user's exact wording and the assistant's full reply as-is; do NOT summarize, paraphrase, or truncate)",
      "timestamp": "ISO8601"
    }
  ],
  "skills": [
...
- `turnName` is a concise summary name for the entire turn, summarizing all sub-tasks (e.g. "Check local weather and plan weekend outing", "Update deployment config and restart service"). Keep it short and descriptive.
- `messages` is an array of chat messages for this turn. Each entry has `role` (`"user"` for user input, `"assistant"` for AI reply), `content` (the **original, verbatim** message text — do not summarize or paraphrase; keep the user's exact words and the assistant's full reply), and `timestamp` (ISO8601). Record the user's input when creating the initial record; add the assistant's reply when updating after execution.
...
This skill operates mostly silently. The only user-visible output is: after a record is successfully saved, briefly remind the user that they can view the task execution record in the **Deck**. Keep the reminder short and natural, and **the reminder language must match the user's language**. For example:
- If the user writes in English: "📋 Task recorded — check the Deck for details."
